...
#    lambda_= np.linalg.inv(J*M_inv* J.T)  #better this if J is not invertible  
    ...
    if time > 1.0:
        #Torque disturbance        
        tauExt = np.multiply(conf.ampJS, np.sin(two_pi_f_JS*time + conf.phiJS))# + np.multiply(conf.ampJS, np.sin(3*two_pi_f_JS*time + 1.57)) #Sine
#        tauExt = np.matrix([20.0, 20.0, 20.0, 20.0, 20.0, 20.0]).T                  #Step
